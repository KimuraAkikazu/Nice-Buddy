        ...
        system_messages = [
            ...
            },
            {
                ...
                "content": (
                    ...
                    "回答の方法を指示します。"
                    "回答は口頭で説明するスピーチ部分と、テキストとして表示するテキスト部分で構成してください。"
                    "基本的に回答は30~50文字程度のスピーチ部分のみで説明してください。"
                    "しかし、もしテキスト部分を生成したほうが良いような内容（プログラムやなにかの詳細な説明など）がある場合は"
                    "前半をスピーチ部分、後半をテキスト部分というように分けて、その区切りは「code102」"
                    "という文字列で行ってください。"
                    "しかし、もしテキスト部分を生成したほうが良いような内容（プログラムやなにかの詳細な説明など）がある場合は"
                    "前半をスピーチ部分、後半をテキスト部分というように分けて、その区切りは「code102」"
                    "という文字列で行ってください。"
                    "しかし、もしテキスト部分を生成したほうが良いような内容（プログラムやなにかの詳細な説明など）がある場合は"
                    "出力の際に必ずテキスト部分があるかどうかを確認し、コードや関数などの自然言語ではない文字をスピーチ部分に含めないでください。"
                    # "なので、スピーチ部分のみを出力する場合は「code102」という文字列は不要です。これは重要なので気をつけてください。"
                    "また、${max_tokens}トークン以内で回答してください。"
                )
            }
        ]
    elif language == "en-US":
